...
def make_prebuilt(prebuilt, freeze_prop=.25, wgh='imagenet', include_ceilometer=False):
    def _prebuilt(img_shape, ceil_shape, labels):
        ...
        if include_ceilometer:
            ...
            y = Dense(16, activation="relu")(ceil_input)
            y = Dropout(0.5)(x)
            x = Concatenate()([x, y])
...
def make_prebuilt_features(prebuilt, freeze_prop=.25):
